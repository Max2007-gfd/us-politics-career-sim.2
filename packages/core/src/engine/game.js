...
export function doAction(state, kind) {
    ...
    if (kind === 'meet-stakeholder') {
        if (s.actors.length === 0)
            return pushLog(s, 'warn', 'No stakeholders loaded.');
    ...
    if (kind === 'solve-case') {
        if (s.caseworkQueue.length === 0)
            return pushLog(s, 'warn', 'No casework in queue.');
    ...
    if (kind === 'draft-policy') {
        if (s.issues.length === 0)
            return pushLog(s, 'warn', 'No issues available.');
        ...
        if (res.ok)
            s = { ...s, stats: { ...s.stats, capital: clamp(s.stats.capital + 1, 0, 100) } };
    }
...
export function meetStakeholder(state, id) {
    if (state.calendar.blocksLeft <= 0)
        return pushLog(state, 'warn', 'No time blocks left this week.');
    const actor = state.actors.find(a => a.id === id);
    if (!actor)
        return pushLog(state, 'warn', `Could not find stakeholder (${id}).`);
    const delta = 5;
    const updatedActors = state.actors.map(a => a.id === id
...
export function draftPolicy(state, issueId, instrument) {
    if (state.calendar.blocksLeft <= 0)
        return pushLog(state, 'warn', 'No time blocks left this week.');
    ...
    s = pushLog(s, res.ok ? 'good' : 'warn', `Drafted ${issue.name} (${instrument}): ${res.ok ? 'OK' : 'Failed'} (${res.notes})`, 'policy');
    if (res.ok)
        s = { ...s, stats: { ...s.stats, capital: Math.min(100, s.stats.capital + 1) } };
    return s;
}
export function solveCase(state, id) {
    if (state.calendar.blocksLeft <= 0)
        return pushLog(state, 'warn', 'No time blocks left this week.');
