...
function getActorRelation(a: { relation?: number; disposition?: number }): number {
  // Use whatever your Actor actually has; many seeds use `disposition`
  return (a.relation ?? a.disposition ?? 0) | 0
}

function canEndorse(state: GameState, id: string): boolean {
  const sh = state.actors.find(
    (s: typeof state.actors[number]) => s.id === id
  )
  if (!sh) return false
...
function makeMoreActors<T extends { id: string; name: string }>(proto: T): T[] {
  return [
...
export function requestEndorsement(state: GameState, id: string): GameState {
  if (state.calendar.blocksLeft <= 0) return pushLog(state, 'warn', 'No time blocks left this week.')

  const actor = state.actors.find(a => a.id === id)
  if (!actor) return pushLog(state, 'warn', `Unknown stakeholder (${id}).`, 'endorsement')
  if (actor.endorsed) return pushLog(state, 'info', `${actor.name} already endorsed you.`, 'endorsement')
  ...
  let s: GameState = { ...state, calendar: consume(state.calendar, 1) }
