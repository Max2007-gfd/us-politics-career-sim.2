...
// Local types for structured logs we append into state.log
type LogLevel = 'info' | 'good' | 'warn' | 'error'
type LogPatch = { level: LogLevel; msg: string; tag?: string }
...
type Store = {
  ...
  // --- New helpers/mechanics ---
  pushLog: (e: LogPatch) => void
  askForEndorsement: (id: string) => void
}
...
export const useGameStore = create<Store>((set, get) => ({
  ...
  clearLog: () => set(s => ({ state: { ...s.state, log: [] as any[] } })),
  ...
  pushLog: ({ level, msg, tag }) => {
    set(s => {
      ...
      const prevLog = ((s.state as any).log ?? []) as any[]
      const nextLog = [entry, ...prevLog].slice(0, 200)
      const nextState = { ...(s.state as any), log: nextLog } as unknown as Core.GameState
      return { state: nextState }
    })
  },

  // ---- New: Endorsements v1 (lightweight) ----
  askForEndorsement: (id: string) => {
    const { state } = get()
    const st = state as any

    const actors = (st.actors ?? []) as any[]
    const idx = actors.findIndex((a: any) => a?.id === id)
    if (idx === -1) {
    ...
    if (score >= threshold) {
      ...
      set(() => ({ state: nextState as Core.GameState }))
